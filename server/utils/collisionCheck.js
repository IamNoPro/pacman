export function collisionCheck({player,wall}){
    ...
    return(
        player.position.z - player.radius + player.velocity.y <= wall.position.z + wall.depth / 2 + padding 
        && player.position.x + player.radius + player.velocity.x >= wall.position.x - padding - wall.width / 2
        && player.position.z + player.radius + player.velocity.y >= wall.position.z - padding - wall.depth / 2
